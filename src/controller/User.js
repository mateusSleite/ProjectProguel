...
class UserController{
    static async register(req, res){
        ...
        try{
            ...
            return res.status(500).send({ message: "Erro ao cadastrar", data: error.message });
        }
    }
    ...
    static async login(req, res){
        ...
        if(!email || !password) return res.status(400).json({ menssage: "E-mail e senha são obrigatórios" });
        ...
        try{
            ...
            if(decryptPassword !== password) return res.status(401).json({ message: "Senha incorreta" });
            ...
            res.status(200).json({ token: token });
                ...
                return res.status(500).json({ message: "Erro ao logar!", data: error.menssage });
            }
    }
}
